...
namespace WebAPI_Sample2.ORM
{
    public class Context
    {
        ...
        public void AddUser(Models.UserInfo data)
        {
            ...
            if (!data.Telfono1.ToReal().IsPhoneNbr())
                throw new Exception(string.Format(@"Numero di telefono errato."));
            if (!data.DataNasc.ToReal().CheckDataNasc()) throw new Exception(string.Format(@"Data nascita errata."));
            users.Add(data); 
